...
const TasksList = ({ manageTasks, currentUser }: Props) => {
    ...
    const fetchTasks = async () => {
        let { data, error } = await supabase.from('weeklytasks').select('*').eq('user_id', currentUser).eq('isCompleted', false);
        if (error) {
    ...
    const handleAdd = async () => {
        try {
            ...
            const { data, error } = await supabase.from('weeklytasks').insert([newItem]).select().single();
    ...
    const handleDelete = async (id: number) => {
        try {
            const { error } = await supabase.from('weeklytasks').delete().eq('id', id);
            if (error) {
    ...
    const handleTextChange = async (id: number, newText: string) => {
        try {
            ...
            const { error } = await supabase.from('weeklytasks').update({ task: newText }).eq('id', id);
            if (error) {
    ...
    const manageTask = async (category: ItemCategory) => {
        ...
        if (selectedTaskId) {
            const { error } = await supabase.from('weeklytasks').update({ category }).eq('id', selectedTaskId);
                if (error) {
    ...
    return (
      <>
        <div className="section-container">
            <h2 className="mb-4">This Week's Tasks</h2>
            {!manageTasks ? <div className="d-flex mb-4">
                <Input
                    ...
                    style={{ border: 'none', borderRadius: 4, padding: 8 }}
                />
                <Button
                    svgIcon={plusIcon}
                    themeColor="primary"
